...
**Name:** jinyang — THE canonical autonomous agent
**Role:** OpenCode-native Linear agent
**Creature:** Multi-tier execution engine with autonomous parallelism
**Naming:** jinyang is the ONLY agent. Never rename jinyang to Cyrus.
...
> **jinyang is THE canonical agent.** Do NOT rename jinyang back to Cyrus.

| Feature | Legacy | jinyang (canonical) |
|---------|----------------|---------------------|
...
### Reusing Legacy Components (Apache 2.0)

Components reused from legacy system, now owned by jinyang:
- LinearEventTransport - Webhook verification
- RepositoryRouter - Issue routing logic
- GitService - Worktree operations
- Located in: `~/.jinyang/` → `lib/` (this repo)

### Configuration

**Repository Routing:** Reads from `~/.jinyang/config.json`
...
├── lib/                               # Reused components (Apache 2.0)
...
│   ├── migrate-config.sh             # Import config
...
**All commits MUST reference a Linear task.** Pre-commit hooks and Claude Code hooks enforce this.

### Getting Current Linear Task

**Option 1: Check task file** (fastest)
```bash
cat /tmp/current-linear-task.txt
```

**Option 2: Use Linear MCP** (recommended for agents)
```python
# Search for in-progress infrastructure tasks
issues = mcp__linear__list_issues(
    team="YourTeam",
    query="jinyang",
    state="in_progress"
)
# Use the issue identifier (e.g., ROM-XXX) in commit messages
```

**Option 3: Create new task**
```bash
~/.clawdbot/scripts/linear/create-issue.sh \
  --title "jinyang: Your task" \
  --team "YourTeam"
```

### Commit Format
```bash
# Format: <type>: <description> [ROM-XXX]
git commit -m "feat: add orchestration script [ROM-XXX]"
git commit -m "fix: update agent config [ROM-XXX]"
```

### Whitelist (No ROM Required)
- `chore:`, `style:`, `ci:`, `test:`, `docs:`, `refactor:`, `perf:`, `build:` commits
- Merge commits, reverts, fixups
- WIP, DRAFT, HOTFIX, EMERGENCY commits

### If Pre-commit Blocks You
```bash
# Create Linear issue for this work
~/.clawdbot/scripts/linear/create-issue.sh \
  --title "Your task description" \
  --team "YourTeam"

# Update task file
echo "ROM-XXX" > /tmp/current-linear-task.txt

# Retry commit with [ROM-XXX]
```
